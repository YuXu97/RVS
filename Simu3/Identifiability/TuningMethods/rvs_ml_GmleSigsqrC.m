function [EstInfo] = rvs_ml_GmleSigsqrC(data, n, M, kernel, method)

[lb,ub] = lubounds_GmleSigsqrC(kernel, M);
...
if strcmp(kernel, 'TC_bd') || strcmp(kernel, 'TC_opt+') || strcmp(kernel, 'TC_tc+')...
    ...
    % hp = [a1 a2 ... aM c lam]
    ratio = 10;
    lb = [-4*ones(M,1); -25;  0.01; -25];
    ub = [4*ones(M,1); 10; 0.99; 10];
...
if strcmp(kernel, 'DC_bd') || strcmp(kernel, 'NEW_bd')...
    ...
    ratio = 10;    
    lb = [-4*ones(M,1); -25;  0.01; -1; -25];
    ub = [4*ones(M,1); 15; 0.99; 1; 10];
    fun = @(x)nglglklhd(x, Psi, y, kernel, M, method);
    ms = MultiStart('FunctionTolerance',1e-5,'UseParallel',false);
    ms.StartPointsToRun='bounds';
    ms.Display='off';
