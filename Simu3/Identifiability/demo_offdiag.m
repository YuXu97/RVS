...
parfor repi =1:Maxrepi % 1:20 %
    ...
    for kk = 1:length(kernel)
        ...
        if strcmp(kernel{kk}, 'TC_opt') || strcmp(kernel{kk}, 'TC_tc')...
            ...
            EstInfo1 = rvs_ml_GmleSigsqr(data,  n,  M, [kernel{kk},'+'], method);
            EstInfo2 = rvs_ml_GmleSigsqr(data,  n,  M, [kernel{kk},'-'], method);
            ...
            EstInfo = rvs_ml_GmleSigsqr(data,  n,  M, kernel{kk}, method);
            hyper = EstInfo.hp;
            Ov = CalculateOutputKernel(CalculatePsi(uv,n),Psi, M, kernel{kk}, hyper, 0);
        end
...
save('Results/SIGSQR_gmle.mat','SIGSQR')